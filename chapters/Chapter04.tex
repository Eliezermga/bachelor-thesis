...
Le Tableau~\ref{tab:resultats_metriques}  met en évidence des écarts de performance significatifs entre les trois modèles évalués.

Le modèle \textbf{\ac{NLLB}-200-distilled-600M} obtient les meilleurs résultats dans les deux directions de traduction (\ac{BLEU} de 37,55 et \ac{chrF} de 57,25 en Ruund$\rightarrow$Français ; \ac{BLEU} de 26,02 et \ac{chrF} de 53,95 en Français$\rightarrow$Ruund). Cette supériorité peut s'expliquer par la couverture linguistique nettement plus large de ce modèle, pré-entraîné sur plus de 200~langues, incluant un nombre important de langues africaines à faibles ressources. Cette exposition préalable à des structures linguistiques typologiquement proches du ruund (langues bantoues ou agglutinantes) favorise vraisemblablement un \textbf{transfert cross-lingual} plus efficace lors du \textit{fine-tuning}, conformément aux mécanismes de transfert décrits dans la littérature \cite{rakhimov2024enhancing}. La version distillée du modèle permet en outre de conserver cette capacité de généralisation tout en réduisant les besoins computationnels.

Le modèle \textbf{\ac{mBART}-50} obtient des performances intermédiaires (\ac{BLEU} de 20,84 à 25,23 selon la direction). Sa couverture linguistique plus restreinte (50~langues, majoritairement à fortes ressources) limite probablement l'exposition du modèle à des structures morphologiques comparables à celles du ruund avant le \textit{fine-tuning}, réduisant d'autant l'efficacité du transfert de connaissances.

Le modèle \textbf{\ac{MarianMT}} affiche les performances les plus faibles (\ac{BLEU} de 10,64 à 15,56), en particulier dans la direction Français$\rightarrow$Ruund. Contrairement aux deux modèles précédents, MarianMT repose sur des architectures pré-entraînées pour des paires de langues spécifiques, sans couverture massive multilingue. Le \textit{fine-tuning} sur le ruund s'effectue donc avec une base de connaissances linguistiques nettement plus limitée, ce qui se traduit par un apprentissage proche d'un entraînement partiel \textit{from scratch} et explique la faiblesse des scores obtenus, en cohérence avec les difficultés déjà documentées pour l'entraînement de systèmes neuronaux sur des corpus réduits.

Un constat mérite par ailleurs d'être souligné : à l'exception de \ac{mBART}-50, les modèles \ac{NLLB} et \ac{MarianMT} obtiennent de meilleurs scores en direction Ruund$\rightarrow$Français qu'en Français$\rightarrow$Ruund, ce qui suggère que la génération vers le français, langue à forte ressource déjà bien représentée dans le pré-entraînement, reste globalement plus aisée que la génération vers le ruund, dont la morphologie agglutinante complexifie la production. Le comportement inverse observé chez \ac{mBART}-50 constitue une anomalie qui ne peut être expliquée avec certitude à partir des données disponibles et mériterait une investigation complémentaire.
