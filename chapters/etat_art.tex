...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images/repartition.jpg}
...
\hline
1 & Traduction automatique neuronale pour les langues à faibles ressources & Reconnaissance automatique de la parole (\textit{Speech Recognition}) \\
\hline
2 & Construction de corpus parallèles & Classification de textes \\
\hline
3 & Alignement de phrases & Analyse de sentiments \\
\hline
4 & Prétraitement des données & Recherche d'information (\textit{Information Retrieval}) \\
\hline
5 & Segmentation en sous-mots (\ac{BPE}, \ac{SentencePiece}) & Enquêtes (\textit{surveys}) \\
\hline
6 & Modèles multilingues & Revues de littérature ou études de synthèse \\
\hline
7 & Apprentissage par transfert (\textit{Transfer Learning}) & \\
\hline
8 & Métriques d'évaluation telles que \ac{BLEU}, \ac{chrF} & \\
\hline
9 & Développement de prototypes de traduction automatique & \\
\hline
\end{tabular}
...
\begin{figure}[H]
    ...
    \caption{Diagramme \ac{PRISMA} du processus de sélection des études}
    \label{fig:prisma}
\end{figure}
...
Les sources disponibles pour le ruund sont fragmentaires et non structurées pour le \ac{ML}. L'effort de \textit{Data Engineering} (nettoyage, normalisation, alignement de 8~431 paires) réalisé dans ce travail constitue une contribution majeure à la documentation numérique de cette langue \cite{Mununga2026ruunddataset}.
